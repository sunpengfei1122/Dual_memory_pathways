...
class SpikingLMUFFTCell(LMUFFTCell):
    ...
    def forward(self, x):
        ...
        pre = (x_pre + self.gamma * self.weight(m))      # [B,T,H]
...
class ConvLMU2(nn.Module):
    def __init__(self, dd, TT, t):
        super().__init__()
        self.LMU1 = SpikingLMUFFTCell(input_size=1,   hidden_size=200, memory_size=dd, seq_len=TT, theta=t)
        self.LMU2 = SpikingLMUFFTCell(input_size=200, hidden_size=200, memory_size=dd, seq_len=TT, theta=t)
        self.dense = nn.Linear(in_features=200, out_features=10)
        self.rnn_drop = nn.Dropout(p=0.5)
