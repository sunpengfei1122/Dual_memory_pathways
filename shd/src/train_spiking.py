...
class shdDataset(Dataset):
    ...
    def __getitem__(self, index):  
        ...
        input,_ =  input.max(dim=-1)   # only spike
...
if __name__ == '__main__':
    ...
    dl_train = DataLoader(dataset=trainingSet, batch_size=128, shuffle=True, num_workers=4) 
    testSet = shdDataset('data/test')    
    dl_val = DataLoader(dataset=testSet, batch_size=128, shuffle=False, num_workers=4)     
       
    dd= args.d
    TT = 100
    t = args.t
    model = ConvLMU2(dd, TT, t).to(DEVICE)
    optimizer = optim.AdamW(params = model.parameters(), lr=0.005, weight_decay=1e-1)
